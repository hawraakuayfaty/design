...
// ألوان الشارات مأخوذة من توكنات الثيم الحالية (confirmed/pending/submitted) — لا حاجة
// لبالتة ألوان جديدة لثلاث فئات فقط، بعكس بنود مصاريف السيارات الستة
const EXPENSE_TYPE_META = {
  SALARY: { label: "راتب", tokenKey: "confirmed" },
  BONUS: { label: "مكافأة", tokenKey: "pending" },
  OTHER: { label: "أخرى", tokenKey: "submitted" },
};
...
// month يعود بصيغة "YYYY-MM-01" من راوت الصرف و"YYYY-MM" من فلاتر أخرى — slice(0,7) يوحّد الشكلين
function formatMonthLabel(monthStr) {
  if (!monthStr) return "—";
  const d = new Date(`${String(monthStr).slice(0, 7)}-01T00:00:00`);
  if (isNaN(d.getTime())) return String(monthStr);
  return new Intl.DateTimeFormat("ar", { month: "long", year: "numeric", numberingSystem: "latn" }).format(d);
}
...
function SkeletonRows({ t, rows = 4, cols = 6 }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "6px 0" }}>
      {Array.from({ length: rows }).map((_, ri) => (
        <div key={ri} style={{ display: "flex", gap: 10 }}>
          {Array.from({ length: cols }).map((__, ci) => (
            <SkeletonLine key={ci} t={t} w={ci === 0 ? "16%" : "14%"} h={14} />
          ))}
        </div>
      ))}
    </div>
...
export default function EmployeeReportDashboard({ t, onBack }) {
  ...
  const [tableEmployee, setTableEmployee] = useState(null);
  const [tableRows, setTableRows] = useState([]);
  const [tableTotals, setTableTotals] = useState({ totalAmount: 0, totalCash: 0, totalShamCash: 0 });
  const [tableMeta, setTableMeta] = useState({ total: 0, page: 1, limit: 10, totalPages: 0 });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data } = await employeeAccountingService.getSummary({ from: summaryFrom, to: summaryTo });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setEmployees(list);
          setSelectedEmployeeId((prev) => (prev != null ? prev : list[0]?.employeeId ?? null));
        }
      } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (selectedEmployeeId == null) {
        ...
        setTableMeta({ total: 0, page: 1, limit: 10, totalPages: 0 });
        return;
      }
      ...
      try {
        ...
        const { data } = await employeeAccountingService.getExpenses(selectedEmployeeId, params);
        const payload = data?.data ?? data;
        if (!cancelled) {
          setTableEmployee(payload?.employee || null);
          setTableRows(payload?.data || []);
          setTableTotals(payload?.totals || { totalAmount: 0, totalCash: 0, totalShamCash: 0 });
          setTableMeta(payload?.meta || { total: 0, page: 1, limit: 10, totalPages: 0 });
        }
      } catch {
        if (!cancelled) {
          setTableEmployee(null);
    ...
    return () => { cancelled = true; };
  }, [selectedEmployeeId, filterType, filterPaymentMethod, filterFrom, filterTo, page]);

  const byTypeBreakdown = useMemo(() => {
    const byType = summary?.byType;
    if (!byType) return [];
    const list = ["SALARY", "BONUS", "OTHER"]
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap", marginBottom: 20 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <h2 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: t.text }}>تقرير مصاريف الموظفين</h2>
          </div>
          <p style={{ margin: "8px 0 0", fontSize: 14, color: t.textSec }}>
            عرض وتحليل فقط — رواتب وموظفي الاستقبال والمحاسبة (لا يشمل المدربين)
          </p>
        </div>
      ...
      <div className="dashboard-stack">
        {summaryLoading ? <SkeletonStatsGrid t={t} /> : !summary ? (
          <div style={{ ...cardStyle(t), height: "auto" }}>
            <div style={emptyStateStyle(t)}>تعذر تحميل ملخص مصاريف الموظفين لهذه الفترة</div>
          </div>
        ) : (
          <>
            <div className="dashboard-stats-grid">
              <StatCardEx
                ...
                t={t}
              />
              <StatCard label="عدد الفواتير" value={String(summary.totalCount ?? 0)} color={t.text} icon={<TbReceipt2 size={22} />} t={t} />
              <StatCardEx
                label="إجمالي الرواتب"
                value={formatMoney(summary.byType?.salary?.total)}
                sub={`${summary.byType?.salary?.count ?? 0} فاتورة راتب`}
                color={t.confirmed.text}
                icon={<TbCoins size={22} />}
                t={t}
              />
              <StatCardEx
                label="إجمالي المكافآت"
                value={formatMoney(summary.byType?.bonus?.total)}
                sub={`${summary.byType?.bonus?.count ?? 0} مكافأة`}
              ...
              <div style={{ ...cardStyle(t), height: "auto" }}>
                ...
                <div style={{ display: "flex", height: 10, borderRadius: 6, overflow: "hidden", background: t.bgElevated }}>
                  {byTypeBreakdown.map((r, i) => (
                    <div key={r.type} title={`${EXPENSE_TYPE_META[r.type]?.label}: ${formatMoney(r.total)}`} style={{
                      width: `${r.pct}%`, background: t[EXPENSE_TYPE_META[r.type].tokenKey].text,
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 10 }}>
                  {byTypeBreakdown.map((r) => (
                    <div key={r.type} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: t.textSec }}>
                      <span style={{ width: 8, height: 8, borderRadius: "50%", background: t[EXPENSE_TYPE_META[r.type].tokenKey].text, display: "inline-block" }} />
                      {EXPENSE_TYPE_META[r.type].label} ({r.count})
        ...
        <div style={{ ...cardStyle(t), height: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap", marginBottom: 4 }}>
            <div>
              <div style={cardTitleStyle(t)}><TbReceipt2 size={16} /> كشف مصاريف الموظف</div>
              {tableEmployee && (
                <div style={{ fontSize: 12, color: t.textMuted, marginTop: -8, marginBottom: 10 }}>
                  الراتب الأساسي الحالي: {formatMoney(tableEmployee.monthlySalary)}
                  {tableEmployee.resignDate ? ` — مستقيل بتاريخ ${formatDateOnly(tableEmployee.resignDate)}` : " — على رأس عمله"}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="vrd-filter-bar" style={{ marginBottom: 14 }}>
            ...
            <select value={selectedEmployeeId ?? ""} onChange={(ev) => { setSelectedEmployeeId(ev.target.value ? Number(ev.target.value) : null); setPage(1); }} style={filterInputStyle(t)}>
              {employeesLoading && <option value="">جارٍ التحميل...</option>}
              {!employeesLoading && !employees.length && <option value="">لا يوجد موظفون</option>}
              {employees.map((e) => (
          ...
          {!employeesLoading && !employees.length ? (
            <div style={emptyStateStyle(t)}>لا يوجد موظفون مسجّلون</div>
          ) : selectedEmployeeId == null ? (
            <div style={emptyStateStyle(t)}>اختر موظفاً لعرض مصاريفه</div>
          ) : tableLoading ? (
                    ...
                    <tbody>
                      {tableRows.map((row, i) => (
                        <tr key={row.expenseId} style={{ background: i % 2 === 0 ? t.bgSurface : t.bgPage, borderBottom: `0.5px solid ${t.border}` }}>
                          <td style={{ padding: "10px 12px", color: t.text, fontWeight: 600 }}>{tableEmployee?.name || "—"}</td>
                          <td style={{ padding: "10px 12px" }}><TypeBadge type={row.type} t={t} /></td>
                          <td style={{ padding: "10px 12px", color: t.text, fontWeight: 700 }}>
                            {formatMoney(row.amount)}
                            {row.type === "SALARY" && row.deduction != null && (
                              <div style={{ fontSize: 11, color: t.textMuted, fontWeight: 500 }}>
                                {formatMoney(row.baseSalary)} − {formatMoney(row.deduction)}
                              </div>
                            )}
                          </td>
