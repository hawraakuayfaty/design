...
import { generalExpensesService, employeesService, employeeAccountingService, accountingService, expensesService } from "./api";
...
function PgGeneralExpenses({ t }) {
  ...
  return (
    <div style={{ padding: "20px 24px", overflowY: "auto", flex: 1, position: "relative" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...thSt, textAlign: "center" }}>إجراءات</th>
                </tr>
              </thead>
              <tbody>
                {rows.map(row => (
                  <tr key={row.expenseId} style={{ transition: "background 0.1s" }} onMouseEnter={e => e.currentTarget.style.background = t.bgElevated} onMouseLeave={e => e.currentTarget.style.background = ""}>
                    ...
                    <td style={{ ...tdSt, textAlign: "center" }}>
                      <div style={{ display: "flex", gap: 4, justifyContent: "center", alignItems: "center" }}>
                        <button onClick={() => setDetailExpId(row.expenseId)} title="تفاصيل الفاتورة" style={{ background: "none", border: "none", cursor: "pointer", color: t.accent, padding: "4px 6px", borderRadius: 6, display: "flex", alignItems: "center" }}>
                          <LuFileText size={15} />
                        </button>
      ...
      {detailExpId != null && (
        <ExpenseDetailModal expenseId={detailExpId} t={t} onClose={() => setDetailExpId(null)} />
      )}
...
const CATEGORY_LABEL_MAP = { GENERAL: "عام", VEHICLE: "مركبة", EMPLOYEE: "موظف", INSTRUCTOR: "مدرب" };
const DISBURSE_STATE_LABEL = { DISBURSER_UNKNOWN: "صُرفت — الصارف غير مسجّل في النظام", NOT_DISBURSED_YET: "لم تُصرف بعد" };
...
function DRow({ k, v, t }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "5px 0", borderBottom: `1px solid ${t.border}`, fontSize: 12 }}>
      <span style={{ color: t.textSec, flexShrink: 0, marginLeft: 12 }}>{k}</span>
      <span style={{ color: t.text, fontWeight: 600, textAlign: "left", direction: "ltr" }}>{v ?? "—"}</span>
    </div>
  );
}

function renderExpDetail(data, t) {
  const { category, detail } = data;
  if (!detail) return null;

  const sec = (title, children) => (
    <div style={{ borderRadius: 9, border: `1px solid ${t.border}`, padding: 14 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: t.textSec, marginBottom: 10, letterSpacing: 0.4 }}>{title}</div>
      {children}
    </div>
  );

  if (category === "VEHICLE") return sec("تفاصيل المركبة", (
    <>
      <DRow k="المركبة"     v={[detail.vehicle?.model, detail.vehicle?.plateNumber].filter(Boolean).join(" — ")} t={t} />
      {detail.vehicle?.type  && <DRow k="نوع المركبة"   v={detail.vehicle.type}       t={t} />}
      {detail.reason         && <DRow k="السبب"          v={detail.reason}             t={t} />}
      {detail.liters != null && <DRow k="الكمية (لتر)"  v={String(detail.liters)}     t={t} />}
    </>
  ));

  if (category === "EMPLOYEE") return sec("تفاصيل الراتب", (
    <>
      <div style={{ padding: "6px 10px", borderRadius: 7, background: t.accentLight, fontSize: 11, color: t.accentText, marginBottom: 8 }}>
        الموظف التالي هو مَن يتقاضى الراتب — المُصرِف (من أجرى الدفع فعلياً) موضّح في قسم الصرف أعلاه
      </div>
      <DRow k="المستفيد (الموظف)"  v={detail.employee?.name}                         t={t} />
      {detail.employee?.phone && <DRow k="هاتف المستفيد"   v={detail.employee.phone} t={t} />}
      {detail.type            && <DRow k="نوع الراتب"       v={detail.type}          t={t} />}
      {detail.month           && <DRow k="الشهر"            v={detail.month}         t={t} />}
      {detail.baseSalary != null && <DRow k="الراتب الأساسي" v={`${fmtAmt(detail.baseSalary)} ل.س`} t={t} />}
    </>
  ));

  if (category === "INSTRUCTOR") return sec("تفاصيل استحقاق المدرب", (
    <>
      <DRow k="نوع الاستحقاق" v={detail.kind === "LESSON" ? "درس" : detail.kind === "BONUS" ? "حافز" : detail.kind} t={t} />
      <DRow k="المدرب"         v={detail.instructor?.name || detail.instructor?.user?.name}                         t={t} />
      {detail.booking && <DRow k="رقم الحجز" v={`#${detail.booking.id ?? detail.booking.bookingId ?? "—"}`} t={t} />}
      {detail.booking?.scheduledAt && <DRow k="تاريخ الحجز" v={new Date(detail.booking.scheduledAt).toLocaleDateString("ar-SY")} t={t} />}
    </>
  ));

  if (category === "GENERAL" && detail.type) return sec("تفاصيل إضافية", (
    <DRow k="النوع التفصيلي" v={EXP_TYPE_MAP[detail.type]?.lbl || detail.type} t={t} />
  ));

  return null;
}

/** @param {{ expenseId: number, t: object, onClose: () => void }} props */
function ExpenseDetailModal({ expenseId, t, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e.response?.data?.message || "فشل تحميل التفاصيل");
      } finally {
  ...
  const state = data?.disbursement?.state;

  return (
    <Modal title={`تفاصيل الفاتورة #${expenseId}`} onClose={onClose} t={t} width={520}>
      {loading ? (
        <div style={{ textAlign: "center", padding: 40, color: t.textMuted }}>جارٍ التحميل...</div>
      ) : error ? (
        <div style={{ padding: "10px 14px", borderRadius: 9, background: "rgba(199,72,72,0.1)", border: "1px solid rgba(199,72,72,0.25)", color: "#c74848", fontSize: 13 }}>{error}</div>
      ) : data && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {/* Core info */}
          <div style={{ background: t.bgElevated, borderRadius: 10, padding: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
            <DRow k="التصنيف"      v={CATEGORY_LABEL_MAP[data.category] || data.category}              t={t} />
            <DRow k="المبلغ"       v={`${fmtAmt(data.amount)} ل.س`}                                   t={t} />
            <DRow k="طريقة الدفع" v={PAY_LABEL[data.paymentMethod] || data.paymentMethod}              t={t} />
            <DRow k="التاريخ"      v={data.expenseDate || data.paidAt?.split("T")[0]}                  t={t} />
            <DRow k="الحالة"       v={data.status === "PAID" ? "مدفوعة ✓" : "غير مدفوعة"}            t={t} />
            {data.note && <div style={{ gridColumn: "1/-1" }}><DRow k="ملاحظات" v={data.note} t={t} /></div>}
          </div>

          {/* Disbursement */}
          <div style={{ borderRadius: 9, border: `1px solid ${t.border}`, padding: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: t.textSec, marginBottom: 10, letterSpacing: 0.4 }}>معلومات الصرف</div>
            {state === "DISBURSED" ? (
              <>
                <DRow k="صُرفت بواسطة"   v={data.disbursedBy?.name}                                          t={t} />
                {data.disbursedBy?.role  && <DRow k="دور الصارف"  v={data.disbursedBy.role}                 t={t} />}
                {data.disbursedBy?.phone && <DRow k="هاتف الصارف" v={data.disbursedBy.phone}                t={t} />}
                {data.disbursement?.at   && <DRow k="وقت الصرف"   v={new Date(data.disbursement.at).toLocaleString("ar-SY")} t={t} />}
              </>
            ) : (
              <div style={{ padding: "8px 12px", borderRadius: 8, fontSize: 13, fontWeight: 600, background: state === "NOT_DISBURSED_YET" ? t.pending.bg : t.expired.bg, color: state === "NOT_DISBURSED_YET" ? t.pending.text : t.expired.text }}>
                {DISBURSE_STATE_LABEL[state] || state}
              </div>
            )}
          </div>

          {/* Category-specific detail */}
          {data.detail != null && renderExpDetail(data, t)}
        </div>
      )}
    </Modal>
  );
